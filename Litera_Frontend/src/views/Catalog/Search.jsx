...
export default function Search() {
  ...
  // Wishlist Handler (Full API)
  const handleAddWishlistDirect = async (book) => {
    if (submitting || !book) return;
    setSubmitting(true);

    try {
      await BookService.simulasikanTambahWishlist(book);
      
      setToast({ 
        type: 'success', 
        message: `"${book.title}" berhasil ditambahkan ke Wishlist!` 
      });
      ...
      <div className="max-w-6xl mx-auto px-6 pt-8 space-y-6">
        ...
        <div className="flex items-center gap-3 w-full">
          <div className="flex-1 relative flex items-center">
            <svg className="absolute left-4 text-slate-400" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                  ...
                  >
                    ...
                    <div className="p-3 relative flex-1">
                      <span className={`absolute top-5 right-5 text-[9px] font-bold px-2 py-0.5 rounded shadow-sm z-10 ${
                        book.stock === 0 ? 'bg-rose-500 text-white' : 
                        book.stock <= 2 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'
                      }`}>
                        {book.stock === 0 ? 'STOK HABIS' : book.stock <= 2 ? 'TERBATAS' : 'TERSEDIA'}
                      </span>

                      <div 
                        className="w-full aspect-[3/4] rounded-lg overflow-hidden bg-slate-50 border border-slate-100 mb-3 cursor-pointer" 
                        onClick={() => navigate(`/catalog/book/${book.id}`)}
                      >
                        <img src={itemCover} alt={book.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-300" />
                      </div>

                      <div className="space-y-1">
                        <h3 
                          className="text-sm font-bold text-[#0c3966] line-clamp-1 leading-snug group-hover:underline cursor-pointer" 
                          onClick={() => navigate(`/catalog/book/${book.id}`)}
                        >
                          {book.title}
                        </h3>
                        ...
                        <p className="text-[11px] text-slate-400 line-clamp-2 leading-relaxed pt-0.5 font-normal">
                          {book.synopsis || book.description || 'Kisah inspiratif dan materi edukatif pilihan berkualitas tinggi.'}
                        </p>
                      </div>
                    </div>
