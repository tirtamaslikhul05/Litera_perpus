...
<<<<<<< HEAD
  // Fetch data profil (sudah termasuk fines & totalFines dari backend)
  const { data: user, loading } = useFetch(
    () => BookService.getUserProfileComplete()
  );

  // Pending returns (bisa diambil dari backend, sementara pakai fallback)
  const hasPendingReturns = user?.pendingReturns?.length > 0 || false;
=======
...
<<<<<<< HEAD
                        <td className="py-4 px-4 text-slate-500 font-medium">{item.dueDate || '-'}</td>
                        <td className="py-4 px-4 text-center">
                          <span className="bg-[#fee2e2] text-rose-700 px-3 py-1 rounded-full font-bold text-[10px]">
                            {item.daysOverdue || 0} Hari
                          </span>
                        </td>
...
<<<<<<< HEAD
                    <p className="text-[10px] text-blue-200/90">Segera proses pengembalian</p>
                  </div>
                </div>
                <span className="text-blue-300 font-bold text-sm transform group-hover:translate-x-1 transition pl-2">➔</span>
              </div>
            )}
...
<<<<<<< HEAD
                  Rp {(user?.totalFines || 0).toLocaleString('id-ID')}
=======
...
<<<<<<< HEAD
                ...
                <span>Dihitung dari Rp 1.000 / hari keterlambatan</span>
              </div>
            </div>
...
<<<<<<< HEAD
                Harap membawa kartu anggota atau tunjukkan halaman ini saat proses pembayaran.
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 py-2.5 px-4 flex items-center justify-around z-30 shadow-[0_-4px_10px_rgba(0,0,0,0.02)]">
        {/* Bottom nav tetap sama seperti kode asli */}
